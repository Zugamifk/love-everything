...
function XY.length(v)
	return math.sqrt(v.x^2+v.y^2)
end

function XY:normalized()
...
function matrix.toXY(m)
	return setmetatable({x=m[1][1], y=m[2][1]}, xy_meta)
end

function xy_meta.__add(a,b)
	return XY(a.x+b.x, a.y+b.y)
end

function xy_meta.__sub(a,b)
	return XY(a.x-b.x, a.y-b.y)
end

xy_meta.__mul = function( a,b )
	if getmetatable( a ) ~= xy_meta then
		return setmetatable({x=a*b.x, y= a*b.y}, xy_meta)
	elseif getmetatable( b ) ~= xy_meta then
		return setmetatable({x=a.x*b, y= a.y*b}, xy_meta)
	end
	return  setmetatable({x=a.x*b.x, y= a.y*b.y}, xy_meta)
end

xy_meta.__div = function( a,b )
	if getmetatable( a ) ~= xy_meta then
		return  setmetatable({x=a/b.x, y= a/b.y}, xy_meta)
	elseif getmetatable( b ) ~= xy_meta then
		return  setmetatable({x=a.x/b, y= a.y/b}, xy_meta)
...
function xy_meta.__tostring(v)
	return "x: "..v.x.." y: "..v.y
end

function xy_meta.__concat(a,v)
